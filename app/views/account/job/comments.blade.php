						...
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									<td>
										@if($comment->job)
										<a href="{{ route('job.show', $comment->job->slug) }}" target="_blank">
											<i class="glyphicon glyphicon-share" style="font-size:0.8em;"></i>
										</a> [{{ $comment->job->location }}] {{ $comment->job->title }}
										@else
										此招聘信息已被删除
										@endif
									</td>
									...
									<td>
										<a href="javascript:void(0)" class="btn btn-xs btn-danger" onclick="modal('{{ route('myjob.deleteComment', $comment->id) }}')">删除评论
										 </a>
									</td>
								</tr>
								@endforeach
